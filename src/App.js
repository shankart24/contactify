...
export default function App() {
	...
	async function fetchAllUsers() {
		...
		setState((state) => ({
			...state,
			allUsers,
			selectedUser: state.selectedUser?.length === 0 ? data?.[0]?.email : state.selectedUser,
			isLoading: false,
		}));
	}
			...
			<MyContext.Provider value={{ state, setState, handleNewContact }}>
				<section className="">
					...
					<div className="max-w-7xl mx-auto mt-8 px-3 sm:px-8">
						<button
							onClick={() => setState((state) => ({ ...state, showContactModal: true }))}
							...
							{state.isLoading ? (
								...
								<>
									{Object.keys(state.allUsers)?.length > 1 ? (
										...
										<p className="tracking-tighter text-md text-center  text-gray-400">No contacts to show</p>
									)}
								</>
							)}
						</div>
					...
					<Transition appear show={state.showContactModal} as={Fragment}>
						<Dialog
							as="div"
							className="fixed inset-0 z-10 bg-gray-900 bg-opacity-70 overflow-y-auto"
							onClose={() => setState((state) => ({ ...state, showContactModal: false }))}
						>
