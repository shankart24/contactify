...
export default function ContactForm({ type, data }) {
	...
	const handleInputChange = (e) => {
		...
		setState((state) => ({ ...state, newContact: { ...state.newContact, [name]: value } }));
	};

	const handleSubmit = (e) => {
		e.preventDefault();
		const { fullName, email, phone, companyName } = state.newContact;
		if (fullName.length !== 0 && email.length !== 0 && phone.length !== 0 && companyName.length !== 0) {
			setState((state) => ({
				...state,
				isLoading: true,
			}));
			try {
				setDoc(
					...
					if (type !== "add" && data?.email !== state.newContact?.email) {
