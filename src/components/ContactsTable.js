		...
		<div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
			<div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
				<table className="min-w-full leading-normal">
					<thead>
						<tr>
							{["Full Name", "Email", "Contact No", "Company Name", "Chat", "Edit", "Delete"].map((item) => (
								<th
									key={item}
									scope="col"
						...
						{Object.keys(state.allUsers).map((userEmail, index) => {
							return (
								state.selectedUser !== userEmail && (
									<ContactRow
										key={userEmail}
										{...{ data: state.allUsers[userEmail], fromEmail: state.selectedUser }}
									/>
								)
							);
						})}
			...
			<tr key={email}>
				...
				<td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
					<p className="text-gray-900 hover:text-gray-700 cursor-pointer whitespace-no-wrap">
						<svg
							onClick={() => handleSetState("showChatModal", true)}
				...
				<td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
					<p className="text-gray-900 hover:text-gray-700 cursor-pointer whitespace-no-wrap">
						<svg
							onClick={() => handleSetState("showEditModal", true)}
							xmlns="http://www.w3.org/2000/svg"
							className="h-5 w-5"
							viewBox="0 0 20 20"
							fill="currentColor"
									...
									<div className="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl">
										<div className="px-4  flex items-center justify-between">
											...
											<button
												...
												onClick={() => handleSetState("showChatModal", false)}
											>
